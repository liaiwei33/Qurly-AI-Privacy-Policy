# Qurly — Privacy Policy (public)

Public repository hosting the [Qurly](https://github.com/liaiwei33/trivia) app privacy policy for App Store Connect and other store listings.

**Live URL (after GitHub Pages is enabled):**  
https://liaiwei33.github.io/Qurly-AI-Privacy-Policy/

## Enable GitHub Pages (one time)

1. Open **Settings → Pages** for this repo.
2. **Source:** Deploy from a branch → **`main`** → **`/ (root)`** → Save.
3. Wait 1–2 minutes; open the URL above.

## Update policy

Edit `index.html`, bump the “Last updated” date, commit, and push to `main`.

The main app codebase stays in the private `trivia` repository.
